...
#include "../314511041/pov_display.hpp"
#include "app_clock_v1.hpp"
#include "app_pong_sim.hpp"
#include "app_vid.hpp"

// menu_v2.cpp 是整個 POV 模擬程式的入口選單。
// 功能流程：
// 1. 先用 OpenCV 畫出選單畫面。
// 2. 使用 POVDisplay::convert() 把選單畫面轉成硬體驅動需要的 APA102 frame 格式。
// 3. 同時保留原本的 POV_Frame 模擬流程，在電腦上預覽旋轉 LED 顯示效果。
// 4. 讀取鍵盤輸入，左右切換選項，上鍵確認，下鍵返回主選單。
// 5. 選到 CLK/VID/GAME 時，透過對應的 .hpp 入口函式啟動外部功能程式。
...
int main() {
    ...
    // POVDisplay 會把 OpenCV BGR 畫布轉成硬體驅動使用的特定資料格式。
    // 這裡只呼叫 convert() 產生 frame 資料；若要直接寫入 /dev/pov_display，可改用 show()。
    POVDisplay::Config pov_cfg;
    pov_cfg.canvas_size = side_len;
    pov_cfg.num_leds = LED_NUM * 2;
    pov_cfg.half_leds = LED_NUM;
    pov_cfg.degree_resolution = NUM_SLICES;
    POVDisplay pov_display(pov_cfg);
    ...
    while (running) {
        ...
        // 每一幀的流程：
        // 1. 畫出 2D menu。
        // 2. 用 POVDisplay 轉成硬體驅動使用的 frame 格式。
        // 3. 轉成本機模擬用 POV buffer。
        // 4. 產生 POV 模擬畫面。
        // 5. 顯示視窗並讀取鍵盤。
        draw_menu_canvas(menu_canvas, menu);
        pov_display.convert(menu_canvas);
