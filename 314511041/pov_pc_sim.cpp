...
// 要發送給硬體的資料結構 (BGR 格式)
struct POV_Frame {
    uint8_t data_A[NUM_SLICES][LED_NUM][3]; 
    uint8_t data_B[NUM_SLICES][LED_NUM][3];
};

// 採樣用的查表 (Sampling LUT)
int lut_x_A[NUM_SLICES][LED_NUM], lut_y_A[NUM_SLICES][LED_NUM];
int lut_x_B[NUM_SLICES][LED_NUM], lut_y_B[NUM_SLICES][LED_NUM];
...
void convert_to_pov_buffer(const cv::Mat& frame_cropped, struct POV_Frame* buffer) {
...
void simulate_pov_display(const struct POV_Frame& buffer, cv::Mat& display_canvas) {
    ...
    for (int i = 0; i < NUM_SLICES; i++) {
        ...
        for (int j = 0; j < LED_NUM; j++) {
            ...
            // 從 Buffer 中取出 BGR 資料
            cv::Scalar color_A(buffer.data_A[i][j][0], 
                               buffer.data_A[i][j][1], 
                               buffer.data_A[i][j][2]);
            ...
            cv::Scalar color_B(buffer.data_B[i][j][0], 
                               buffer.data_B[i][j][1], 
                               buffer.data_B[i][j][2]);
